...
/* AAAAAAAJJJJJJJJJAAAAAAAAAXXXXXXX */

let urlAPI = 'https://jsonplaceholder.typicode.com/posts';
const info = {mail: 'usuario@pepemail.com', clave: 'asd123'};

$('#pruebaAjax').prepend("<button id='btnAPI' class='btn btn-primary rounded-pill'>Enviar INFO a api</button> <div id='ajaxConfirm'></div>");
//creando el event
$('#btnAPI').click(function () { 
   $.post(urlAPI, info,
       function (response, status) {
           if(status === 'success'){
               $('#ajaxConfirm').prepend(`<div>DATOS ENVIADOS OK: <br> user:    ${response.mail} <br> pw:    ${response.clave}</div>`);
           }
       },
...
let urlInterna = "./json/products.json";
$('#pruebaAjax').append("<button id='btnAPIGET' class='btn btn-primary rounded-pill'>Traer productos disponibles de api</button> <div id='ajaxGet'></div>");
$.get(urlInterna, (response, status) => {
    let contador = 0;
    if(status === "success"){
        $('#btnAPIGET').click(function (event) { 
            $("#ajaxGet").append(
                `<div> <tr>${contador+1}</tr>
                producto: ${response[contador].nombre} <br>
                precio:  ${response[contador].precio} <br>
                cantidad en stock: ${response[contador].stock} <br>  
                  </div>`
            );
            contador++;
        });
